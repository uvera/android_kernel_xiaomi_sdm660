...
#define pr_fmt(fmt) "cpu_general_boost: " fmt
...
static unsigned int get_max_boost_freq(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(policy->cpu, cpu_lp_mask))
		freq = CONFIG_MAX_GENERAL_BOOST_FREQ_LP;
	else
		freq = CONFIG_MAX_GENERAL_BOOST_FREQ_PERF;
...
static void __cpu_general_boost_kick_max(struct boost_drv *b,
...
void cpu_general_boost_kick_max(unsigned int duration_ms)
{
	struct boost_drv *b = &boost_drv_g;

	__cpu_general_boost_kick_max(b, duration_ms);
}
...
static int __init cpu_general_boost_init(void)
{
	struct boost_drv *b = &boost_drv_g;
...
subsys_initcall(cpu_general_boost_init);
